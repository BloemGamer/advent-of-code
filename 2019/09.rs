...
fn part2(ic: &Vec<i64>)
{
    ...
    let mut intcode: Vec<i64> = ic.clone();
    intcode.resize(999999, 0);
    let _ = run_intcode(&mut intcode, 2);
    println!("");
}
...
fn run_intcode(intcode: &mut Vec<i64>, input: i64) -> Result<(), ()>
{
    ...
    while i < intcode.len()
    {
        match intcode[i] % 100
        {
            1 => { get_places(i, intcode, &mut places, base_offset); ic_add(&mut i, intcode, &places) },
            2 => { get_places(i, intcode, &mut places, base_offset); ic_mult(&mut i, intcode, &places) },
            3 => { get_places(i, intcode, &mut places, base_offset); ic_input(&mut i, intcode, &places, input) },
            4 => { get_places(i, intcode, &mut places, base_offset); ic_output(&mut i, intcode, &places) },
            5 => { get_places(i, intcode, &mut places, base_offset); ic_jump(&mut i, intcode, &places, true) },
            6 => { get_places(i, intcode, &mut places, base_offset); ic_jump(&mut i, intcode, &places, false) },
            7 => { get_places(i, intcode, &mut places, base_offset); ic_cmp(&mut i, intcode, &places, |a, b| a < b) },
            8 => { get_places(i, intcode, &mut places, base_offset); ic_cmp(&mut i, intcode, &places, |a, b| a == b) },
            9 => { get_places(i, intcode, &mut places, base_offset); ic_adj_offset(&mut i, intcode, &places, &mut base_offset) },
...
fn get_places(i: usize, intcode: &mut Vec<i64>, places: &mut [usize; 4], base_offset: i64)
{
...
fn ic_adj_offset(i: &mut usize, intcode: &mut Vec<i64>, places: &[usize; 4], base_offset: &mut i64)
{
