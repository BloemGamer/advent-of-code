...
A library containing some basic file functions. This library was specifically made for advent of code, but can be used for other purposes. In C these are just functions with a struct, in C++ these are part of a filecontent class. This library should work on both windows and linux.
...
-	`filecontent`; All the functions are in thiss class. This class also contains a 2D string to contain al the info from the file. The struct also contains a `size_t` variable to determine the amount of lines in the file (`amountlines`). And also a `size_t` struct to determine the length of line in the file (`lengthlines`).

## Global variables

-   `filecontent.file` (C); Global because there are some functions that fill this specific struct. In advent of code you only get one file you would use at once while running, so 1 global struct should be enough. In the event that ain't enought you can always just define another `filecontent` struct.
